...
INTEGER,PARAMETER::QDL=SELECTED_REAL_KIND(p=32)
...
!  ********************************************************************
     IMPLICIT NONE
     COMPLEX(KIND=QDL),INTENT(IN)::xx
     COMPLEX(KIND=QDL)::mp_func_li2c
     COMPLEX(KIND=QDL)::yy,lyy,loy,zz,z2,liox
     COMPLEX(KIND=QDL),PARAMETER::zero=cmplx(0e0_16,0e0_16,kind=16),one=cmplx(1e0_16,0e0_16,kind=16)
     REAL(KIND=QDL),DIMENSION(36)::bb
     REAL(KIND=QDL)::pi,pi2o6 ,rex,imx
     INTEGER,INTENT(IN)::iph
     ...
     IF (init.EQ.0) THEN
        ...
        pi = mp_func_pi()
        pi2o6  = pi**2/6e0_16
        IF (mp_func_prec().GT.1e-24_16) THEN
          nn = 18 ! double precision
        ELSE
          nn = 36 ! quadruple precision
        ENDIF
        DO ii=1,nn
          bb(ii) = mp_func_bern(ii)
        ENDDO
     ENDIF
     ...
     IF (imx.EQ.0d0) THEN
        liox = mp_func_li2a(rex,iph)
     ELSE
        rex = ABS(rex)

        IF (iph.EQ.iph/2*2) THEN
          yy = CMPLX(rex,imx,kind=16)
          iyy = iph
        ELSE
           yy = CMPLX(-rex,-imx,kind=16)
          iyy = iph + NINT(SIGN(1e0_16,imx))
       ENDIF

        x_gt_1 = (ABS(xx).GT.1e0_16)
        IF (x_gt_1) THEN
           yy = one/yy
           iyy = -iyy
        ENDIF
        ...
        y_lt_h = (REAL(yy,kind=16).LT.0.5e0_16)
        IF (y_lt_h) THEN
           zz = -loy
        ELSE
           zz = -lyy
        ...
        z2 = zz*zz
        liox = CMPLX(bb(nn),kind=16)
        DO ii=nn,4,-2
           liox = CMPLX(bb(ii-2),kind=16) + liox*z2/(ii*(ii+1))
        ENDDO
        liox = CMPLX(bb(1),kind=16) + liox*zz/3
        liox = zz + liox*z2/2
        IF (y_lt_h) liox = CMPLX(pi2o6,kind=16) - liox - loy*lyy
        !the connection between different Rienman sheets
        liox = liox - loy*CMPLX(0e0_16,pi*iyy,kind=16)
        !li2(1-x)=-li2(1-1/x)-log(x)**2/2
        IF (x_gt_1) liox = -liox - (lyy + CMPLX(0e0_16,iyy*pi,kind=16))**2/2

      ENDIF
      mp_func_li2c = liox
    END FUNCTION mp_func_li2c
...
!  ********************************************************************
      IMPLICIT NONE
      COMPLEX(KIND=QDL)::mp_func_li2a
      COMPLEX(KIND=QDL)::cliox
      REAL(KIND=QDL),INTENT(IN)::xx
      REAL(KIND=QDL),DIMENSION(30)::bb
      REAL(KIND=QDL)::pi,pi2o6,rr,yy,lyy,loy,zz,z2,liox
      INTEGER,INTENT(IN)::iph
      ...
      IF     (rr.EQ.0e0_16) THEN
         cliox = CMPLX(pi2o6,0e0_16,kind=16)
      ELSEIF (rr.EQ.1e0_16.AND.positive) THEN
         cliox = CMPLX(0e0_16,0e0_16,kind=16)
      ELSE
         yy  = rr
         lyy = LOG(rr)
         ...
         IF (r_GT_1) THEN
            ...
            ione = -ione
         ENDIF
        loy = LOG(1e0_16-yy) ! log(1-yy) is always real

        y_lt_h = (yy.LT.0.5e0_16)
        IF (y_lt_h) THEN
        ...
        cliox = CMPLX(liox,kind=16)

        IF (y_lt_h) THEN
          cliox = CMPLX(pi2o6,kind=16) - cliox - CMPLX(loy*lyy , loy*pi*ione,kind=16)
        ENDIF

        cliox = cliox + CMPLX( 0d0 , -loy*pi*ntwo, kind=16)

        IF (r_gt_1) cliox = -cliox - CMPLX( -lyy , iph*pi, kind=16 )**2/2
      ENDIF
      mp_func_li2a = cliox
    END FUNCTION mp_func_li2a
...
!	  INTEGER,EXTERNAL::unit_get
      ...
      FUNCTION mp_func_bern(ii)
!  ********************************************************************
! the first nn Bernoulli numbers
!  ********************************************************************
        IMPLICIT NONE
        REAL(KIND=QDL)::mp_func_bern
        REAL(KIND=QDL),DIMENSION(40)::bern
        INTEGER,INTENT(IN)::ii
        INTEGER::init=0,jj
        ...
        IF (init.EQ.0) THEN
           ...
           DO jj=3,nn-1,2
              bern(jj) = 0e0_16
           ENDDO
           bern( 1) = -1e0_16/2e0_16
           bern( 2) =  1e0_16/6e0_16
           bern( 4) = -1e0_16/30e0_16
           bern( 6) =  1e0_16/42e0_16
           bern( 8) = -1e0_16/30e0_16
           bern(10) =  5e0_16/66e0_16
           bern(12) = -691e0_16/2730e0_16
           bern(14) =  7e0_16/6e0_16
           bern(16) = -3617e0_16/510e0_16
           bern(18) =  43867e0_16/798e0_16
           bern(20) = -174611e0_16/330e0_16
           bern(22) =  854513e0_16/138e0_16
           bern(24) = -236364091e0_16/2730e0_16
           bern(26) =  8553103e0_16/6e0_16
           bern(28) = -23749461029e0_16/870e0_16
           bern(30) =  8615841276005e0_16/14322e0_16
           bern(32) = -7709321041217e0_16/510e0_16
           bern(34) =  2577687858367e0_16/6e0_16
           bern(36) = -26315271553053477373e0_16/1919190e0_16
           bern(38) =  2929993913841559e0_16/6e0_16
           bern(40) = -261082718496449122051e0_16/13530e0_16
        ENDIF
        if (ii.LE.nn) THEN
           mp_func_bern = bern(ii)
        ELSE
           IF (unit_get().GT.0) WRITE(unit_get(),*)'ERROR in mp_func_bern: bernoulli(',ii &
                ,') not yet implemented'
        ENDIF
      END FUNCTION mp_func_bern
...
!  ********************************************************************
        ...
        IF (init.EQ.0) THEN
           init = 1
           pi = 4e0_16*ATAN(1e0_16)
        ENDIF
        mp_func_pi = pi
      END FUNCTION mp_func_pi
...
!	  INTEGER,EXTERNAL::unit_get
      ...
      FUNCTION mp_func_prec()
!  ********************************************************************
! the smallest number  prec  satisfying  1+prec = dexp(dlog(1+prec))
!  ********************************************************************
        IMPLICIT NONE
        REAL(KIND=QDL)::mp_func_prec,prec,xx,yy
        INTEGER::init=0
        SAVE init,prec
        ...
        IF (init.EQ.0) THEN
           ...
           DO WHILE (xx.EQ.yy)
              prec = xx
              xx = xx/2
              yy = -1e0_16+EXP(LOG(1e0_16+xx))
           ENDDO
           !IF (unit_get().GT.0) WRITE(unit_get(),*)'MESSAGE from mp_func_prec: precision set to',prec
        ENDIF
        mp_func_prec = prec
      END FUNCTION mp_func_prec
