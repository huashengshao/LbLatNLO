...
!
! $Id: assertions.f90,v 1.1 2001/06/27 13:40:16 gsalam Exp (changed by HSS) $
!
!======================================================================
module assertions
  ...
  interface default_or_opt
     MODULE PROCEDURE default_or_opt_dp, default_or_opt_sp,&
          default_or_opt_int, default_or_opt_log,default_or_opt_dp_array,&
          default_or_opt_sp_array, default_or_opt_int_array, default_or_opt_log_array
  end interface
  public :: default_or_opt

contains

  function default_or_opt_dp_array(xdef, xopt) result(x)
    real(dp),dimension(:),intent(in)::xdef
    real(dp),dimension(size(xdef)),intent(in),optional::xopt
    real(dp),dimension(size(xdef))::x
    integer::n
    n=size(xdef)
  ...
  end function default_or_opt_dp_array
  ...
  function default_or_opt_sp_array(xdef, xopt) result(x)
    real(sp),dimension(:),intent(in)::xdef
    real(sp),dimension(size(xdef)),intent(in),optional::xopt
    real(sp),dimension(size(xdef))::x
    integer::n
    n=size(xdef)
    if(present(xopt))then
       x(1:n)=xopt(1:n)
  ...
  end function default_or_opt_sp_array
  ...
  function default_or_opt_int_array(xdef, xopt) result(x)
    integer,dimension(:),intent(in)::xdef
    integer,dimension(size(xdef)),intent(in),optional::xopt
    integer,dimension(size(xdef))::x
    integer::n
    n=size(xdef)
    if(present(xopt))then
       x(1:n)=xopt(1:n)
  ...
  end function default_or_opt_int_array
  ...
  function default_or_opt_log_array(xdef, xopt) result(x)
    logical,dimension(:),intent(in)::xdef
    logical,dimension(size(xdef)),intent(in),optional::xopt
    logical,dimension(size(xdef))::x
    integer::n
    n=size(xdef)
    if(present(xopt))then
       x(1:n)=xopt(1:n)
  ...
  end function default_or_opt_log_array
