...
! quadruple precision of lagrange_basis_function_1d
  subroutine lagrange_basis_function_1d_qp(mx,xd,i,xi,yi)
    !*****************************************************************************80
    !
    !! LAGRANGE_BASIS_FUNCTION_1D evaluates one 1D Lagrange basis function.
    !
    ...
    !    Input, integer ( kind = 4 ) MX, the degree of the basis function.
    !
    !    Input, real ( kind = 16 ) XD(MX+1), the interpolation nodes.
    !
    !    Input, integer ( kind = 4 ) I, the index of the basis function.
    !    1 <= I <= MX+1.
    !
    !    Input, real ( kind = 16 ) XI, the evaluation point.
    !
    !    Output, real ( kind = 16 ) YI, the value of the I-th Lagrange 1D basis
    !    function for the nodes XD, evaluated at XI.
    !
    implicit none    
    integer ( kind = 4 ) mx
    ...
    return
  end subroutine lagrange_basis_function_1d_qp

  ! quadruple precision version of lagrange_interp_2d
  subroutine lagrange_interp_2d_qp( mx, my, xd_1d, yd_1d, zd, ni, xi, yi, zi )
    !*****************************************************************************80
    !
    !! LAGRANGE_INTERP_2D evaluates the Lagrange interpolant for a product grid.
    !
    ...
    !    Input, integer ( kind = 4 ) MX, MY, the polynomial degree in X and Y.
    !
    !    Input, real ( kind = 16 ) XD_1D(MX+1), YD_1D(MY+1), the 1D data locations.
    !
    !    Input, real ( kind = 16 ) ZD((MX+1),(MY+1)), the 2D array of data values.
    !
    !    Input, integer ( kind = 4 ) NI, the number of 2D interpolation points.
    !
    !    Input, real ( kind = 16 ) XI(NI), YI(NI), the 2D interpolation points.
    !
    !    Output, real ( kind = 16 ) ZI(NI), the interpolated values.
    !
    implicit none
    ...
    real ( kind = 16 ) lx
    real ( kind = 16 ) ly
    real ( kind = 16 ) xd_1d(mx+1)
    real ( kind = 16 ) xi(ni)
    real ( kind = 16 ) yd_1d(my+1)
    real ( kind = 16 ) yi(ni)
    real ( kind = 16 ) zd(mx+1,my+1)
    real ( kind = 16 ) zi(ni)
    ...
    do k = 1, ni
       ...
       do i = 1, mx + 1
          do j = 1, my + 1
             l = l + 1
             call lagrange_basis_function_1d_qp( mx, xd_1d, i, xi(k), lx )
             call lagrange_basis_function_1d_qp( my, yd_1d, j, yi(k), ly )
             zi(k) = zi(k) + zd(i,j) * lx * ly
          end do
       end do
    end do
    ...
    return
  end subroutine lagrange_interp_2d_qp
