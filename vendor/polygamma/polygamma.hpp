...
// pGamma(m, z) = (-1)^m pGamma(m, 1-z) - Pi^(1+m)*(d^m/dy^m*Cot(y)) |_y=Pi*z
...
std::complex<T> mdCot(int m, std::complex<T> z) {
	...
	while (m >= (int)p.size()) {
	        // p_{n+1}(z) = -(1+z^2)p'_n(z), degree of p_n is n+1 (size n+2)
	        // derives p_{n+1} from p_n
	        // p.size()=n+1, c = n
	        int c = p.size()-1;
...
std::complex<T> pGamma(int m, std::complex<T> z) {
	...
	// There is another large |z| relation eq.(6) in arXiv:0902.2148
	// They are probably same (TBC)
